...
async def fetch_image(url):
    try:
        ...
        return b'\x00\x00\xFF' * 32 * 32 * 3

async def fetch_images(offset=0, limit=30):
    start_index = offset
    end_index = min(start_index + limit, 133)  # Ensure it doesn't go beyond the total number of images

    urls = [f"https://api.slingacademy.com/public/sample-photos/{i}.jpeg" for i in range(start_index + 1, end_index + 1)]
...
async def serve_composite_image(request):
    rows, cols = 10, 3  # Assuming a grid layout
    offset = 30  # Set the offset to 30 images
    images = await fetch_images(offset=offset, limit=30)
